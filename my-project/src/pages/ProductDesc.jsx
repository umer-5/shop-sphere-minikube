...
const ProductDesc = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                setLoading(true);
                const { data } = await axios.get(`http://192.168.49.2:31000/api/v1/product/get-product/${slug}`);
                if (data?.success) {
    ...
    const getSimilarProducts = async (pid, cid) => {
        try {
            const { data } = await axios.get(`http://192.168.49.2:31000/api/v1/product/related-products/${pid}/${cid}`);
